...
class HKDF_Tests(unittest.TestCase):
    _testData = (
            # Test cases from RFC 5869 Appendix A.1 to A.7
            (SHA256,
    ...
    def test1(self):
        """Test cases taken from RFC 5869 Appendix A"""
        for record in self._testData:
            hashAlgo, ikm, salt, info, expected_okm = record
            ikm = t2b(ikm)
            salt = t2b(salt)
            info = t2b(info)
            expected_okm = t2b(expected_okm)
            dkLen = len(expected_okm)
