...
class PCTBuildExt (build_ext):
    ...
    def detect_modules (self):
        ...
        if self.compiler.compiler_type == 'msvc':
            ...
            build_temp_dir = os.path.join(self.build_temp, 'src')
            if not os.path.exists(build_temp_dir):
                os.makedirs(build_temp_dir)

            # Visual studio 2010 and earlier version missing `stdint.h`
            # Visual studio 2013, 2015, 2017 implement it.
            try:
                self.compiler.compile(['src/inc-msvc/check_stdint.c'], build_temp_dir)
            except distutils.errors.DistutilsExecError as e:
                self.compiler.include_dirs.insert(0, "src/inc-stdint/")
                try:
                    print('warning: mising "stdint.h", try "src/inc-stdint/stdint.h" instead!')
                    self.compiler.compile(['src/inc-msvc/check_stdint.c'], build_temp_dir)
                except distutils.errors.DistutilsExecError as e:
                    print('Error: compile check_stdint.c fail')
                    sys.exit(-1)
